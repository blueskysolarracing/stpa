...
def query_equality(
    ...
    content = content.strip().strip('"').rstrip('.').upper()

    try:
        response = YesOrNoResponse(content)
    except ValueError:
        response = YesOrNoResponse.NO

    return response == YesOrNoResponse.YES


_RAW_EQUALITY_QUERY_PROMPT = '''
Are the STPA Definitions saying similar things?
...
def query_raw_equality(
        client: OpenAI,
        model: str,
        raw_definition_0: str,
        raw_definition_1: str,
        seed: int | None = None,
        temperature: int | None = None,
) -> bool:
    prompt = _RAW_EQUALITY_QUERY_PROMPT.format(
        raw_definition_0,
        raw_definition_1,
    )
    completion = client.chat.completions.create(
...
An unsafe control action must contain the following five parts: <Source>\
 <Type> <Control Action> <Context> <Link to Hazards>
...
def classify_raw_unsafe_control_action_quality(
    ...
    try:
        ...
        response = QualityResponse.INCORRECT
...
def generate_filtered_raw_unsafe_control_actions(
        client: OpenAI,
        model: str,
        auxiliary_model: str,
        count: int,
    ...
    filtered_raw_unsafe_control_actions = list[str]()
    raw_unsafe_control_actions = list[str]()

    while len(filtered_raw_unsafe_control_actions) < count:
        while not raw_unsafe_control_actions:
        ...
        raw_unsafe_control_action = raw_unsafe_control_actions.pop()
        quality = classify_raw_unsafe_control_action_quality(
            client,
            model,
            unsafe_control_actions,
            raw_unsafe_control_action,
        ...
        if quality == QualityResponse.CORRECT_AND_USEFUL:
            for filtered_raw_unsafe_control_action in (
                    filtered_raw_unsafe_control_actions
            ):
                equality = query_raw_equality(
                    client,
                    auxiliary_model,
                    raw_unsafe_control_action,
                    filtered_raw_unsafe_control_action,
                    seed,
                    temperature,
                )
                ...
                filtered_raw_unsafe_control_actions.append(
                    raw_unsafe_control_action,
                )

                yield raw_unsafe_control_action


def generate_filtered_raw_scenarios(
        client: OpenAI,
        model: str,
        auxiliary_model: str,
        count: int,
    ...
    filtered_raw_scenarios = list[str]()
    raw_scenarios = list[str]()

    while len(filtered_raw_scenarios) < count:
        while not raw_scenarios:
        ...
        raw_scenario = raw_scenarios.pop()
        quality = classify_raw_scenario_quality(
            client,
            model,
            scenarios,
            raw_scenario,
        ...
        if quality == QualityResponse.CORRECT_AND_USEFUL:
            for filtered_raw_scenario in filtered_raw_scenarios:
                equality = query_raw_equality(
                    client,
                    auxiliary_model,
                    raw_scenario,
                    filtered_raw_scenario,
                    seed,
                    temperature,
                )
                ...
                filtered_raw_scenarios.append(raw_scenario)
